...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Returns:
            The result of the evaluated expression or an error message if invalid
        """
        try:
            # Here we validate the expression for safety
            if not re.match(r'^[\d\s\+\-\*\/\(\)\.]+$', expression):
                raise ValueError("Invalid characters in expression. Only numbers and basic operators are allowed.")

            # To evaluate the expression safely
            result = eval(expression, {"__builtins__": None}, {})

            # Log the tool use with the ToolLogger function
            logger.log_tool_use(
                "calculator",
