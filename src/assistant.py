...
#TODO: Complete the config dictionary to set the thread_ud, llm, and tools to the workflow
        ...
        config = {
            "configurable": {
                "thread_id": self.current_session.session_id,  # Set this to the session id of the current session
        ...
        try:
            ...
            err_str = str(e)
            # Detect shared/Vocareum budget exhaustion and provide actionable guidance
            if "Insufficient budget" in err_str or "Exceeded budget" in err_str or "budget" in err_str:
                user_msg = (
                    "OpenAI API error: insufficient budget on the configured endpoint. "
                    "This repository may be using a shared/test endpoint whose quota is exhausted. "
                    "To fix this, provide your own OpenAI API key with billing enabled by setting the "
                    "environment variable `OPENAI_API_KEY` or pass a valid key when creating `DocumentAssistant`. "
